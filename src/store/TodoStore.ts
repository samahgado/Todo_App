...
export const useTodoStore = create<ListsState>()(
  devtools(
    persist(
      (set) => ({
        lists: testLists,
        activListId: null,
        selectedListId: (id) => set(() => ({ activListId: id })),
        addList: (list) => set((state) => ({ lists: [...state.lists, list] })),
        addTodo: (todo) =>
          set((state) => ({
            lists: state.lists.map((list) =>
              list.id === state.activListId
        ...
        deleteList: () =>
          set((state) => ({
            lists: state.lists.filter((list) => list.id !== state.activListId),
          })),
